...
function TrendingFacebook() {
  ...
  useEffect(() => {
    // Fetch summary data
    axios.get("http://127.0.0.1:5000/api2/summary")
      .then(response => setSummaryData(response.data))
      .catch(error => console.error("Error fetching summary data:", error));

    // Fetch time-series data
    axios.get("http://127.0.0.1:5000/api2/time_series")
    ...
    axios.get("http://127.0.0.1:5000/api2/music_genre")
    ...
    axios.get("http://127.0.0.1:5000/api2/audience")
      .then((response) => {
        const audience = response.data;

        // Count occurrences of each trend
        const audienceCounts = {};
        Object.values(audience).forEach((trend) => {
          audienceCounts[trend] = (audienceCounts[trend] || 0) + 1;
        });

        // Convert to array and sort by count
        const sortedTrends = Object.entries(audienceCounts)
          .sort((a, b) => b[1] - a[1]) // Sort by count in descending order
          .slice(0, 5); // Get top 5 trends

        setaudience(sortedTrends);
      })
      .catch((error) => console.error("Error fetching trends:", error))
      .finally(() => setLoading(false));
    
    axios.get("http://127.0.0.1:5000/api2/trend")
      .then((response) => {
        const trend = response.data;

        // Count occurrences of each trend
        const trendCounts = {};
        Object.values(trend).forEach((trend) => {
  ...
  const totalAudience = audience.reduce((sum, [, count]) => sum + count, 0);
  ...
  const pieChartData = {
    labels: audience.map(([trend]) => trend),
    datasets: [
      {
        label: 'Trend Type Percentage',
        data: audience.map(([, count]) => ((count / totalAudience) * 100).toFixed(2)),
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
          <h2 className="text-xl font-semibold mb-4">Top 5 Trends</h2>
          {trend.map(([trend, count], index) => (
...
<div className="bg-gray-800 p-6 rounded-xl shadow-lg">
  <h2 className="text-xl font-semibold mb-4 ">Audience Demographics Percentage Distribution</h2>
  {audience && (
...
export default TrendingFacebook;
